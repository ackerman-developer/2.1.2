...
class VacancySet:
    def __init__(self, vacancy: Dict[str, str]):
        ...
        self.salary = SalaryCount(salary_from=vacancy["salary_from"],
...
class ParamSalary:
    def __init__(self, param : str, salary: SalaryCount):
    ...
    def add_salary(self, new_salary : SalaryCount):
        self.count_vacancy += 1
...
class InputConnect:
    ...
    def __process_data(self) -> None:
        data = DataBase(self.input_data[0]).vacancies_objects
        ...
        graph = Graph(profession=profession,
    ...
    def __convert_to_param_salary(self, vacancies: List[VacancySet], comparison_param: str) -> (List[ParamSalary]):
        param_salary = {}
        for vacancy in vacancies:
    ...
    def __add_missing_years(self, param_salary: List[ParamSalary], year_salary : (Dict[int, int], Dict[int, int])) -> List[ParamSalary]:
        ...
        for y in years:
            if y not in s_years:
                param_salary.insert(int(y) - int(years[0]), ParamSalary(y, SalaryCount("0", "0", "RUR")))
